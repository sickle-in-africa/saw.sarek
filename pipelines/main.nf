...
process Output_documentation {
    ...
    script:
    """
    ${params.sarekDir}/bin/markdown_to_html.py $output_docs -o results_description.html
    """
}
...
workflow.onComplete {
    ...
    def tf = new File("${params.sarekDir}/assets/email_template.txt")
    def txt_template = engine.createTemplate(tf).make(email_fields)
    def email_txt = txt_template.toString()

    // Render the HTML template
    def hf = new File("${params.sarekDir}/assets/email_template.html")
    def html_template = engine.createTemplate(hf).make(email_fields)
    def email_html = html_template.toString()

    // Render the sendmail template
    def smail_fields = [ email: email_address, subject: subject, email_txt: email_txt, email_html: email_html, projectDir: "$projectDir", mqcFile: mqc_report, mqcMaxSize: params.max_multiqc_email_size.toBytes() ]
    def sf = new File("${params.sarekDir}/assets/sendmail_template.txt")
